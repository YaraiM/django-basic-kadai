...
# 画像ファイル関連の設定
# settingsにある変数をインポートする。（ローカル環境）
from django.conf import settings
# 静的ファイル（画像など）を読み込めるようにする
from django.conf.urls.static import static

urlpatterns = [
    #adminへのルーティングはデフォルトでDjangoに記載されている
...
# 開発モード（DEBAG＝True）限定の設定
# 画像を静的ファイルとして扱い、ローカル（MEDIA_URL, MEDIA_ROOT）にルーティング
# 本番環境ではアプリの性能・安全性向上のため、画像をローカルではなく別の場所に設定して提供する
if settings.DEBUG:
